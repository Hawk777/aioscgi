...
def _lifespan_wrapper(
    application: ApplicationType, never: Awaitable[None]
) -> ApplicationType:
    """
    ...
    async def impl(
        ...
        try:
            ...
            # protocol locally, so the LifespanManager can just assume lifespan is
            # always supported.
            crashed = True
...
class LifespanManager:
    ...
    def __init__(
        ...
        :param started: A callable that LifespanManager invokes once the application has
        ...
        :param shutdown_complete: A callable that LifespanManager invokes once the
            application has shut down, passing the failure message if shutdown failed or
            None if shutdown succeeded.
        """
        self._wrapped_application = _lifespan_wrapper(application, never)
        self._never = never
        self._state = state
    ...
    async def run(self: LifespanManager) -> Any:
        """
    ...
    async def _receive_gen(self: LifespanManager) -> AsyncIterator[EventOrScope]:
    ...
    async def _receive(self: LifespanManager) -> EventOrScope:
        """Receive the next lifespan event."""
        async with self._receive_mutex:
            return await anext(self._receive_iter)

    async def _send(self: LifespanManager, event: EventOrScope) -> None:

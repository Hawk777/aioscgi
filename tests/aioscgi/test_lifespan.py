...
@pytest.mark.asyncio
async def test_lifespan_startup_successful() -> None:
    ...
    # The lifespan manager should keep running after a successful start, so cancel it.
    uut_future.cancel()
    await uut_future
...
@pytest.mark.asyncio
async def test_lifespan_shutdown_successful() -> None:
    ...
    # At this point, startup should have finished, but shutdown should not have started.
    assert not shutdown_seen
    assert not shutdown_complete_called
